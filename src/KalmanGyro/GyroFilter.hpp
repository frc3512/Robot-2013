...
class GyroFilter {
public:
    GyroFilter(double (GyroBase::*angleFunc)(), double (GyroBase::*rateFunc)(),
               GyroBase* funcObj);
    virtual ~GyroFilter();
...
private:
    ...
    double
        m_Q_angle;  // Process noise variance for the accelerometer/magnetometer
    ReentrantSemaphore m_Q_angleSem;
    double m_Q_bias;  // Process noise variance for the gyro bias
    ReentrantSemaphore m_Q_biasSem;
    double m_R_measure;  // Measurement noise variance - this is actually the
                         // variance of the measurement noise
    ReentrantSemaphore m_R_measureSem;

    double m_angle;  // The angle calculated by the Kalman filter - part of the
                     // 2x1 state matrix
    ReentrantSemaphore m_angleSem;
    double m_rate;  // Unbiased rate calculated from the rate and the calculated
                    // bias - you have to call getAngle to update the rate
    ReentrantSemaphore m_rateSem;
    double m_bias;  // The gyro bias calculated by the Kalman filter - part of
