...
GraphHost::GraphHost(int port)
    : m_inst(GraphHost_create(port)), m_sendInterval(5) {
    resetTime();
}

GraphHost::~GraphHost() { GraphHost_destroy(m_inst); }
...
int GraphHost::graphData(float value, std::string dataset) {
    ...
    return GraphHost_graphData(m_currentTime - m_startTime, value,
                               dataset.c_str(), m_inst);
}
